...
import firebaseService from '../api/firebaseService'; // Import your firebaseService
...
const AudioForm = ({ onSubmit }) => {
    ...
    const [audioFileName, setAudioFileName] = useState(''); // Store file name
    ...
    const handleAudioChange = (e) => {
        ...
        if (file) {
            setAudioFile(file);
            setAudioFileName(file.name); // Store the file name
        } else {
            setAudioFile(null);
            setAudioFileName('');
        }
    ...
    const onSubmitForm = async (data) => {
        setSubmitError(null);
        if (!audioFile) {
            alert("Please select an audio file."); // Basic validation
            return;
        }
        if (!selectedGenre) {
        ...
        const audioData = {
            ...
            duration: parseInt(data.duration, 10), // Ensure duration is an integer
            imageUrl: null,
            fileUrl: null, // Placeholder, will be filled after upload
        };
        try {
          await onSubmit(audioData, audioFile, imageFile); // Pass both files.
    ...
    <Box component="form" onSubmit={handleSubmit(onSubmitForm)} sx={{ mt: 2 }} noValidate>
      ...
      <TextField
        ...
        {...register("duration", {
          ...
          valueAsNumber: true, // Treat input as a number
          validate: (value) =>
